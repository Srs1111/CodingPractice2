...
const formatDueDateForResponse = date => format((new Date(date), 'yyyy-MM-dd'))
...
const convertDueDate = dbObject => {
  return {
    ...
    dueDate: formatDueDateForResponse(dbObject.due_date),
  }
}
...
app.get('/todos/:todoId/', async (request, response) => {
  ...
  if (todo) {
    ...
    response.status(404).send('Invalid due date')
  }
})
